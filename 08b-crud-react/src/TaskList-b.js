...
export default function TaskList (){
    ...
    // This is one more way of doing the edit, but more complex;
        ...
        return (
            tasks.map((t) => (
                ...
                (<li key={t.id}>
                    ...
                    />
                    {!toggleActive ?
                    <button onClick={()=> {setTaskBeingEdited(t.id); setModifiedTaskName(t.description); setToggleActive(true); }}> Edit </button>
                    : (<span></span>)
                    } 
    ...
    function displayEditTask(t){
        return (
            <li key={t.id}>
                ...
                />
                <button onClick={()=>{updateTask(t.id); setTaskBeingEdited(0); setModifiedTaskName(''); setToggleActive(false);}}> Confirm </button>
                <button onClick={()=> deleteTask(t.id)}> Delete </button>
        </li>
        )
    }
